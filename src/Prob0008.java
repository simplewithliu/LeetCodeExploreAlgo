...
提示：
本题中的空白字符只包括空格字符 ' ' 。
假设我们的环境只能存储 32位大小的有符号整数，那么其数值范围为 [−2^31,  2^31 − 1]。
...
public class Prob0008 {
	class Solution {
		public int myAtoi(String s) {
			...
			int idx = 0, res = 0, sign = 1, len = s.length();
			...
			while (idx < len && s.charAt(idx) >= '0' && s.charAt(idx) <= '9') {
				if (Integer.MAX_VALUE / 10 < res || (Integer.MAX_VALUE / 10 == res && Integer.MAX_VALUE % 10 < (s.charAt(idx) - '0'))) {
